...
shutil.copytree(msys / 'lib' / 'gdk-pixbuf-2.0', libdir / 'gdk-pixbuf-2.0',
                dirs_exist_ok=True)
...
for dep in datadeps:
    shutil.copytree(msys / 'share' / dep, datadir / dep,
                    dirs_exist_ok=True)

# Copy gettext packages
linguas_file = source_dir / 'po' / 'LINGUAS'
linguas = [l.strip() for l in open(linguas_file).readlines()
                     if not l.startswith('#')]
gettext_packages = [
    'gdk-pixbuf',
    'gettext-runtime',
...
for lang in linguas:
    for pkg in gettext_packages:
        ...
        if mo_file.exists():
            ...
            shutil.copy(mo_file, datadir / 'locale' / lang / 'LC_MESSAGES' / f"{pkg}.mo")
        else:

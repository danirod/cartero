...
def create_dmg(path: Path):
    ...
    args = ["create-dmg",
            "--volname", "Cartero",
            "--volicon", self_dir / "DmgIcon.icns",
            "--window-size", "854", "480",
            "--background", self_dir / "DmgBackground.png",
            "--icon-size", "96",
            "--icon", "Cartero.app", "210", "225",
            "--app-drop-link", "640", "225",
            "--eula", self_dir / ".." / "GPL-3.0.rtf",
            path / "Cartero.dmg", path]
    subprocess.run(args)

def notarize(profile: str, path: Path):
    args = ["xcrun", "notarytool", "submit", path, "--keychain-profile", profile, "--wait"]
    subprocess.run(args)
...
parser.add_argument('-p', '--profile', help='The keychain profile to use when notarizing')
